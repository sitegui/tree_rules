...
type Tree = {
    tagName: string
    children: Map<string, Tree | string>
}
...
function buildTree(rules: Rule[], remainingTagNames: Set<string>): Tree {
    // Study how good each tag cuts the rules
    const candidates: {tagName: string, rulesByValue: Map<string, Rule[]>, totalDiversity: number}[] = []
    for (const tagName of remainingTagNames) {
        // Partition by a tag
        const rulesByValue: Map<string, Rule[]> = new Map()
...
function measureDiversity(labels: string[]): number {
